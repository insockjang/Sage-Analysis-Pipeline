require(devtools)
source_url("https://raw.githubusercontent.com/insockjang/Sage-Analysis-Pipeline/master/PathwayAnalysis/preRankedTest.R")
...
myPathwayAnalysis <- setRefClass(Class = "myPathwayAnalysis",                                 
                                  ...
                                  methods = list(
                                    ...
                                    fet = function(AllGenes,geneSet,testSet){ 
                                      ...
                                      fisher.result <- fisher.test(Mat2x2, alternative = "greater")   
                                      .self$fetResult <- list(p.value = fisher.result$p.value,stats = fisher.result$estimate, mat = Mat2x2, fetGeneSet = intersect(testSet,geneSet), geneSet = geneSet)
                                    },
